...
export default function InvestmentReportPage() {
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">
          ...
          <p className="text-gray-600">Please generate an investment report first.</p>
        </div>
      </div>
    );
  }
  ...
  const lumpsumMF = investmentData.lumpsum_mutual_funds || investmentData.mutual_funds || [];
  ...
  const lumpsumBonds = investmentData.lumpsum_bonds || investmentData.bonds || [];
  const monthlyBonds = investmentData.monthly_bonds || [];

  const totalLumpsum =
    investmentData.total_lumpsum != null ? investmentData.total_lumpsum : sumVals(lumpsumAlloc);
  const totalMonthly =
    investmentData.total_monthly != null ? investmentData.total_monthly : sumVals(monthlyAlloc);

  const reportId = investmentData.id || investmentData._id || investmentData.task_id;
  ...
  return (
    <div className="container mx-auto p-6 max-w-screen-2xl">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Investment Portfolio Report</h1>
      ...
      <div id="report" className="bg-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <div className="p-4 bg-purple-50 rounded-xl border border-purple-200 text-center">
            <div className="text-xs font-semibold text-purple-700 uppercase">Total Lumpsum</div>
            <div className="text-xl font-extrabold text-purple-900 mt-1">{INR(totalLumpsum)}</div>
          </div>
          <div className="p-4 bg-blue-50 rounded-xl border border-blue-200 text-center">
            <div className="text-xs font-semibold text-blue-700 uppercase">Total Monthly</div>
            <div className="text-xl font-extrabold text-blue-900 mt-1">{INR(totalMonthly)}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          <div className="p-4 rounded-xl border border-gray-100 shadow-sm">
            <h2 className="text-lg font-semibold text-gray-800 mb-2">Lumpsum Allocation</h2>
            <AllocationPie allocations={lumpsumAlloc} />
          </div>
          <div className="p-4 rounded-xl border border-gray-100 shadow-sm">
            <h2 className="text-lg font-semibold text-gray-800 mb-2">Monthly Allocation</h2>
        ...
        {(lumpsumMF.length > 0 || monthlyMF.length > 0) && (
          <div className="mb-10">
            <h2 className="text-xl font-bold text-gray-900 mb-3">Mutual Funds</h2>
            {lumpsumMF.length > 0 && (
              <div className="mb-6">
                <h3 className="text-sm font-semibold text-gray-700 mb-2">Lumpsum Picks</h3>
                <MutualFundReturns mutualFunds={lumpsumMF} />
              </div>
            )}
            {monthlyMF.length > 0 && (
              <div>
                <h3 className="text-sm font-semibold text-gray-700 mb-2">Monthly SIP Picks</h3>
        ...
        <div className="mt-8 p-6 bg-gray-50 rounded-xl text-center">
          <p className="text-sm text-gray-600">
            This report is generated based on current market data and recommendations. Please
            consult a financial advisor before making investment decisions.
          </p>
          {reportId && <p className="text-xs text-gray-500 mt-2">Report ID: {reportId}</p>}
        </div>
      </div>
    </div>
  );
